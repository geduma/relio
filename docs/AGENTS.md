...
7. On success: `recordSuccess()`, `enqueueLog()`, `enqueueMetric()` — the log records `ttft_ms` (time to first chunk, `null` when no content ever arrived) and, when the upstream includes a `usage` field in a final SSE chunk, `input_tokens`/`output_tokens` (captured by a pass-through `StreamUsageTracker` transform inserted between the adapter stream and `res`; chunks are forwarded byte-for-byte unchanged). The relay never buffers or rewrites the stream.
   - The relay stays **provider-agnostic**: bodies sent upstream are sanitized by standard endpoint schemas only. `sanitizeChatBody()` (in `src/adapters/messageSerializer.js`) forwards only standard OpenAI chat-completions request fields (`model`, `messages`, `stream`, `stream_options`, sampling params, `tools`, `response_format`, `user`, …); `sanitizeEmbeddingsBody()` allows only `model`, `input`, `encoding_format`, `dimensions`, `user`. Anything else a client sends — including `signal` or other transport/control keys — is never forwarded upstream. There is no provider-specific code anywhere in this path.
   - Token capture works for every provider type: the `openai-compatible` adapter auto-requests `stream_options.include_usage: true` (OpenAI/Ollama) but **retries the request once without that field** when the upstream rejects it (HTTP 400/422 whose error mentions `stream_options`), so strict OpenAI-compatible endpoints keep working with no config; the `azure-openai` adapter does the same but only when the effective `api-version` supports it (`>= 2024-10-21`, else graceful degradation), and the `anthropic`/`gemini-native` adapters already map their native usage (`message_delta` / `usageMetadata`) into the OpenAI-style SSE chunks the tracker reads.
