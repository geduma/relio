...
const SECTIONS = [
  ...
  },
  {
    ...
    fields: [
      ...
      {
        key: 'relay.streamTimeoutSeconds',
        label: 'Stream timeout (s)',
        type: 'number',
        half: true,
        desc: 'Hard total ceiling for streaming requests, counted from request start. When it elapses the stream is aborted without penalizing the provider.',
      },
      {
        key: 'relay.streamIdleTimeoutMs',
        label: 'Stream idle timeout (ms)',
        type: 'number',
        half: true,
        desc: 'Abort a stream when no real provider data arrives for this long. Only provider data resets it (keep-alives do not). An idle abort means the upstream is dead and counts as a single circuit-breaker failure; client disconnects and the max-duration cap never count against the provider.',
      },
      {
        ...
        type: 'number',
        half: true,
        desc: 'Send an SSE keep-alive comment to the client when no provider data has arrived for this long (0 = disabled). It does not reset the idle timeout, which still detects a dead upstream. Keep it below the idle timeout to be useful.',
      },
      {
