...
| `relay.streamKeepAliveMs` | Send an SSE keep-alive comment to the client when no provider data has arrived for this long, keeping long-lived streams alive through proxies/clients with idle timeouts (default `15000`; `0` disables). It does **not** reset the idle timeout, which still detects a dead upstream |
