...
| `relay.streamTimeoutSeconds` | Hard total ceiling for streaming requests, counted from request start (default `300`). When it elapses the stream is aborted (`max duration exceeded`) without penalizing the provider |
| `relay.streamIdleTimeoutMs` | Abort a stream if no **real upstream data** arrives for this long (default `30000`). Only provider data resets it — SSE keep-alive comments do **not**. An idle abort is the signal that the upstream is dead, so it counts as a single circuit-breaker failure. A client disconnect or the max-duration cap never counts against the provider |
| `relay.streamKeepAliveMs` | Send an SSE keep-alive comment to the client when no provider data has arrived for this long, keeping long-lived streams alive through proxies/clients with idle timeouts (default `15000`; `0` disables). It does **not** reset the idle timeout, so a dead upstream is still detected by `streamIdleTimeoutMs`. Keep this below `streamIdleTimeoutMs` or it serves no purpose |
